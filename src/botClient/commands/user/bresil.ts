	...
	return {
		...
		async execute(interaction) {
			try {
				...
				const guild = interaction.client.guilds.cache.find(g => g.name === client.config.serverName)
				...
				client.dbClient.selectFromDB("SELECT * FROM bot_bresil WHERE id = ? ", [memberid], (err, rows) => {
					if (err) {
						...
						client.dbClient.updateDB(
							"INSERT INTO bot_bresil (id, username, bresil_received) VALUES (?, ?, 1)",
				...
				client.dbClient.selectFromDB("SELECT * FROM bot_bresil WHERE id = ?", [interactionMember.id], (err, rows) => {
					if (err) {
						...
						client.dbClient.updateDB(
							"INSERT INTO bot_bresil (id, username, bresil_sent) VALUES (?, ?, 1)",
							[interactionMember.id, interactionMember.user.username],
							err => {
								if (err) {
									console.log(err)
								}
