        ...
        async execute(interaction) {
            try {
                ...
                if (!arg || isNaN(arg))
                    return interaction
                        .reply({
                            content: `👎 **You forgot to add the seeking-time!** Usage: \`/seek <Time-In-S>\``
                        })
                        .catch(err => Logger.error(err.message))
                ...
                interaction
                    .reply({
                        content: `⏩ **Seeked to \`${formatDuration(newPos)}\`**!`
                    })
                    .catch(err => Logger.error(err.message))
            } catch (e: any) {
                ...
                interaction
                    .reply({
                        content: `❌ Could not join your VC because: \`\`\`${e.interaction || e}`.substring(0, 1950) + `\`\`\``
                    })
                    .catch(err => Logger.error(err.message))
            }
        }
