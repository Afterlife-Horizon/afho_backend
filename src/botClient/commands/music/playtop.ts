        ...
        async execute(interaction) {
            ...
            try {
                ...
                if (!song && !playlist) return interaction.editReply(`❌ **Failed looking up for ${track}!**`);
                    ...
                    return interaction.editReply(`▶️ **Queued at \`1st\`: __${video.title}__** - \`${video.durationFormatted}\``).catch((err) => console.log(err));
                }
                ...
                console.error(err);
                return interaction.reply(`❌ Could not play the Song because: \`\`\`${err.message || err}`.substring(0, 1950) + `\`\`\``).catch((err) => console.log(err));
            }
        },
    }
};