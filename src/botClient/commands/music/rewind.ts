        ...
        async execute(interaction) {
            try {
                ...
                if (!arg || isNaN(arg))
                    return interaction
                        .reply({
                            content: `👎 **You forgot to add the rewinding-time!** Usage: \`/rewind <Time-In-S>\``
                        })
                        .catch(err => Logger.error(err.message))
                ...
                if (Number(arg) < 0 || Number(arg) > Math.floor(curPos / 1000 - 1)) {
                    return interaction
                        .reply({
                            content: `👎 **The Rewind-Number-Pos must be between \`0\` and \`${Math.floor(curPos / 1000 - 1)}\`!**`
                        })
                        .catch(err => Logger.error(err.message))
                }
                ...
                const resource = client.musicHandler.getResource(queue, queue.tracks[0].id, newPos)
                ...
                interaction
                    .reply({
                        content: `⏪ **Rewinded for \`${arg}s\` to \`${formatDuration(newPos)}\`**!`
                    })
                    .catch(err => Logger.error(err.message))
            } catch (e: any) {
                ...
                interaction
                    .reply({
                        content: `❌ Could not join your VC because: \`\`\`${e.message || e}`.substring(0, 1950) + `\`\`\``
                    })
                    .catch(err => Logger.error(err.message))
            }
        }
