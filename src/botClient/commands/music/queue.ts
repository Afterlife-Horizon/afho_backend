...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, Colors, EmbedBuilder, GuildMember, SlashCommandBuilder } from "discord.js"
...
export default (client: BotClient): ICommand => {
    return {
        ...
        async execute(interaction) {
            ...
            const queue = client.musicHandler.queues.get(interaction.guild.id)
            ...
            const queueEmbed = new EmbedBuilder()
                ...
                .setDescription(
                    `**CURRENT:** \`${song.durationFormatted}\` - [${song.title}](${getYTLink(song.id ? song.id : "")}) - ${song.requester}`
                )
                .addFields(
                    {
                        name: "**Track-loop:**",
                        value: `> ${e2n(queue.trackloop)}`,
                        inline: true
                    },
                    {
                        name: "**Queue-loop:**",
                        value: `> ${e2n(queue.queueloop)}`,
                        inline: true
                    },
                    {
                        name: "**Autoplay:**",
                        value: `> ${e2n(queue.autoplay)}`,
                        inline: true
                    }
                )
                .addFields(
                    queue.tracks
                        ...
                        .map((track, index) => {
                            return {
                                name: `Track \`${client.musicHandler.queuePos(index + 1)}\` - \`${track.durationFormatted}\``,
                                value: `> [${track.title}](${getYTLink(track.id ? track.id : "")}) - ${track.requester}`,
                                inline: false
                            }
                        })
                )
                ...
                await ninteraction.update({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setDescription(
                                `**CURRENT:** \`${song.durationFormatted}\` - [${song.title}](${getYTLink(song.id ? song.id : "")}) - ${
                                    song.requester
                                }`
                            )
                            .addFields(
                            ...
                            .addFields(
                                queue.tracks
                                    ...
                                    .map((track, index) => {
                                        return {
                                            name: `Track \`${client.musicHandler.queuePos(index + i - 9)}\` - \`${track.durationFormatted}\``,
                                            value: `> [${track.title}](${getYTLink(track.id ? track.id : "")}) - ${track.requester}`,
                                            inline: false
                                        }
                                    })
