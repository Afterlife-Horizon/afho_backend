        ...
        async execute(interaction) {
            try {
                ...
                if (!arg || isNaN(arg))
                    return interaction
                        .reply({
                            content: `👎 **You forgot to add the forwarding-time!** Usage: \`/forward <Time-In-S>\``
                        })
                        .catch(err => Logger.error(err.message))
                ...
                const ressource = client.musicHandler.getResource(queue, queue.tracks[0].id, newPos)
                ...
                interaction
                    .reply({
                        content: `⏩ **Forwarded for \`${arg}s\` to \`${formatDuration(newPos)}\`**!`
                    })
                    .catch(err => Logger.error(err.message))
            } catch (e: any) {
