    ...
    return (
        router.post("/", async (req, res) => {
            try {
                ...
                const botsVoiceChanel = guild.channels.cache.find(c => c.type === 2 && c.members.filter((m: GuildMember) => m.user.username === "AFHO_bot").size !== 0);
                ...
                if (!playList) {
                    ...
                    res.status(200).send("OK");
                    return channel.send(`Queued at \`${client.queuePos(queue.tracks.length - (playList.videos.length - 1))}\`: __${video.title} - \`${video.durationFormatted}\`\n> Added \`${playList.videos.length - 1} Songs\` from the Playlist:\n> ${playList.title}`);
                }
            }
            catch (err) {
                console.log(err);
