...
export default function (client: BotClient) {
    return (
        router.post("/", async (req, res) => {
            const guild = client.guilds.cache.find((g: { name: string; }) => g.name === "Afterlife Horizon");
            ...
            const voiceChannel = guild.channels.cache.find((c: Channel) => c.type === 2 && c.members.filter((m: GuildMember) => m.user.username === req.body.user).size !== 0);
