...
export default function (client: BotClient) {
    return router.post("/", async (req, res) => {
        ...
        try {
            ...
            const queue = client.musicHandler.queues.get(currentChannel.guild.id)
            ...
            if (!song && !playlist) {
                res.status(406).send({
                    error: "Failed looking up for ${track}!"
                })
            ...
            if (!playlist) {
                ...
                queue.tracks = [
                    queue.tracks[0],
                    client.musicHandler.createSong(video, user.data.user?.user_metadata.full_name),
                ...
                playlist.videos.forEach(nsong => playlistSongs.push(client.musicHandler.createSong(nsong, user.data.user?.user_metadata.full_name)))
                queue.tracks = [
                    queue.tracks[0],
                    client.musicHandler.createSong(video, user.data.user?.user_metadata.full_name),
                    ...playlistSongs,
