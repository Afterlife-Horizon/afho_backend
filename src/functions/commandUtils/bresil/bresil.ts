...
export default async function bresil(client: BotClient, mover: GuildMember, moved: GuildMember) {
    try {
        ...
        if (!movedVoiceState)
            return {
                status: 406,
                error: "The user you want to move is not in a channel!"
            }
        ...
        if (movedVoiceState.channel?.id === client.config.brasilChannelID)
            return {
                status: 406,
                error: "You cannot bresil someone already in bresil!"
            }
        ...
        if (!isVoiceChannel(brasilChannel))
            return {
                status: 406,
                error: "Brasil channel is not a voice channel!"
            }

        await moved.voice.setChannel(brasilChannel)
        return {
            status: 200,
            message: `You have moved ${moved.user.username} to the brasil channel!\n You have moved ${moverCount} ${
                moverCount > 1 ? "people" : "person"
