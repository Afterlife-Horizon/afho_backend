...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            if (!arg || isNaN(arg)) return interaction.reply({ content: `👎 **You forgot to add the forwarding-time!** Usage: \`/forward <Time-In-S>\`` }).catch((err) => console.log(err));

            if (Number(arg) < 0 || Number(arg) > Math.floor((queue.tracks[0].duration - curPos) / 1000 - 1)) {
                return interaction.reply({ content: `👎 **The Forward-Number-Pos must be between \`0\` and \`${Math.floor((queue.tracks[0].duration - curPos) / 1000 - 1)}\`!**` }).catch((err) => console.log(err));
            }
            ...
            interaction.reply({ content: `⏩ **Forwarded for \`${arg}s\` to \`${interaction.client.formatDuration(newPos)}\`**!` }).catch((err) => console.log(err));
        }
        catch (e) {
            console.error(e);
