...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            if (!arg || isNaN(arg)) return interaction.reply({ content: `👎 **You forgot to add the rewinding-time!** Usage: \`/rewind <Time-In-S>\`` }).catch(() => null);
            ...
            interaction.reply({ content: `⏪ **Rewinded for \`${arg}s\` to \`${interaction.client.formatDuration(newPos)}\`**!` }).catch(() => null);
        }
        catch (e) {
            console.error(e);
