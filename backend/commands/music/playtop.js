...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!track) return interaction.reply(`👎 Please add the wished Music via: \`/playtop <Name/Link>\``).catch(() => null);
        ...
        try {
            ...
            console.error(e);
            return interaction.reply(`❌ Could not play the Song because: \`\`\`${e.message || e}`.substring(0, 1950) + `\`\`\``).catch(() => null);
        }
    },
};